 ...
 * @returns "reverse map"
 */
export async function initLocalization2(): Promise<Map<string, string>> {
...
export async function reverseLookup2(term: string): Promise<string | null> {
  return reverseMap.get(term) ?? null;
}

//
export async function getReverseLookupMap2(): Promise<Map<string, string>> {
  return reverseMap;
}
